...
from app.schemas.report_schemas import ReportRequest, ReportGenerationResponse, ReportStatusResponse
...
@router.post("/generate",
             ...
             description="사용자의 요청(지역, 카테고리 리스트)을 받아 리포트 생성을 Celery Worker에게 요청하고 즉시 작업 ID를 반환합니다.")
def request_report_generation(request: ReportRequest):
    # .model_dump()를 통해 Pydantic 모델을 Celery가 처리할 수 있는 dict 형태로 변환하여 전달
    task = generate_report_task.delay(request.model_dump()) # model_dump(exclude_unset=False)를 사용하여 None 값도 명시적으로 포함 가능
